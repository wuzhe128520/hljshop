...
Class.pt.callback = function(){
  ...
  //按钮
  layero.find('.'+ doms[6]).children('a').on('click', function(e){
	  e.stopPropagation();
  ...
  if(config.shadeClose){
    $('#layui-layer-shade'+ that.index).on('click', function(e){
      layer.close(that.index);
      e.stopPropagation();
    });
  } 
  ...
  //最小化
  layero.find('.layui-layer-min').on('click', function(e){
  ...
  //全屏/还原
  layero.find('.layui-layer-max').on('click', function(e){
